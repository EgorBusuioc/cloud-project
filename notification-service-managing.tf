# Variable for specifying the number of additional Notification servers
variable "additional_notification_count" {
  description = "Number of additional Notification servers to deploy"
  type        = number
  default     = 0  # By default, no additional servers are created
}

# Dynamic Notification servers (auto-created)
resource "openstack_networking_port_v2" "auto_notification_ports" {
  count = var.additional_notification_count

  name               = "port-notification-auto-${count.index + 1}"
...
# VMs for additional Notification servers
resource "openstack_compute_instance_v2" "auto_notification_vms" {
  count = var.additional_notification_count

  name            = "instance-notification-auto-${count.index + 1}"
  ...
  network {
    port = openstack_networking_port_v2.auto_notification_ports[count.index].id
  }
...
echo "Starting user_data script for auto-notification-${count.index + 1}"
...
echo "Starting Notification container: auto-notification-${count.index + 1}"
sudo docker run -d --name auto-notification-${count.index + 1} -p 8080:8080 \
  ...
  busuiocegor/cloud-project:notification-service

echo "Container auto-notification-${count.index + 1} started. Checking status..."
sudo docker ps
echo "User data script for auto-notification-${count.index + 1} completed."
EOF
}

# Additional output data for new servers
output "additional_notification_ips" {
  value = {
    for i, port in openstack_networking_port_v2.auto_notification_ports :
      "auto-notification-${i + 1}" => port.all_fixed_ips
  }
  description = "IP addresses for additional Notification servers"
}